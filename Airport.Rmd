...
```
I want to try the colour for different types of cancellation codes here(A,B and C) but adding that to the aes changes my graph and makes it weird.
```{r}
...
ggplot(data = group, aes(x = dep_time_period, y = count)) +
        ...
        facet_wrap(~UniqueCarrier) + 
        coord_flip()  +
        labs(title = "Normalised Count of Cancelled Flights by Time of Day for each Airline Carrier") + xlab("Time of Day") + ylab("Normalised Count of Cancelled Flights")
```
```{r}
group = austin_airport %>%
group_by(dep_time_period, UniqueCarrier)  %>% 
...
ggplot(data = group, aes(x = dep_time_period, y = avg)) +
        ...
        facet_wrap(~UniqueCarrier) + 
        coord_flip()  +
        labs(title = "Normalised Mean of Delayed Flights by Time of Day for each Airline Carrier") + xlab("Time of Day") + ylab("Normalised Mean of Delayed Flights")
```
Trying to find cancelled flights by the different types of cancellation. Don't know how to remove the blank value.
```{r}
group = austin_airport %>%
...
ggplot(data = group, aes(x = dep_time_period, y = count)) +
        ...
        coord_flip()  +
        labs(title = "Count of Cancelled Flights by Time of Day for each Type of Cancellations") + xlab("Time of Day") + ylab("Count of Cancelled Flights")
```
